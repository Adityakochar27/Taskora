      ...
      <div className="card p-3 sm:p-4 flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex border border-slate-200 rounded-lg overflow-hidden">
          <button
            ...
            className={`px-3 py-2 text-sm flex items-center gap-1 ${view === 'board' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`}
          >
            <LayoutGrid size={14} /> Board
          </button>
          <button
            onClick={() => setView('list')}
            className={`px-3 py-2 text-sm flex items-center gap-1 ${view === 'list' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`}
          >
...
function CreateTaskModal({ open, onClose, onCreated, currentUser }) {
  const [form, setForm] = useState({
    title: '', description: '', assignTo: 'user',
