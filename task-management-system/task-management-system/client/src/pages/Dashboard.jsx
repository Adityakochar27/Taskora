...
export default function Dashboard() {
  ...
  const { data: prod } = useFetch(
    () =>
      user?.role !== 'Employee'
        ? dashboardService.productivity(30)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total tasks" value={s.total} icon={ListTodo} />
        <StatCard
          label="In progress"
          value={s.byStatus?.['In Progress'] || 0}
          icon={Clock}
          accent="amber"
        />
        <StatCard
          ...
          icon={CheckCircle2}
          accent="emerald"
          hint={`${s.completionRate ?? 0}% completion`}
        />
        <StatCard
          label="Overdue"
          value={s.overdue}
      ...
      {/* Per-department (Admin only, populated by API) */}
              ...
              <tbody className="divide-y divide-slate-100">
                {s.perDepartment.map((row) => (
                  <tr key={row._id || 'none'}>
                    ...
                    <td className="py-2 pl-2 text-right text-red-600">
                      {row.overdue}
                    </td>
                  </tr>
                ))}
